...
MAX_LINES = 1000
DEFAULT_ROOTS = (Path("src"), Path("tests"), Path("scripts"), Path("ios/CameraGPSLink"))


def source_files(roots: list[Path]) -> list[Path]:
    files: set[Path] = set()
    for root in roots:
        if root.is_file() and root.suffix in {".py", ".swift"}:
            files.add(root)
            continue
        if not root.exists():
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Reject Python/Swift program sources over 1000 lines.")
