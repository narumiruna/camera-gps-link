...
Status: The software portion is delivered in [Draft PR #17](https://github.com/narumiruna/camera-gps-link/pull/17) on `narumi/feat/ios-reliability-release-readiness`, based on `origin/main` at `b51685fe9b51a89daa7a03d291204ec49de2b18d`. Signed implementation commit `42ac9efac9968ac3e7f7e78a3f4ac31f1bb3a9b7` passes both the local gate and hosted CI. The user has now made the repository public and selected GitHub for the document links; link implementation and renewed verification are in progress. Physical qualification remains blocked; this plan is not complete.
...
- [ ] Add Privacy Policy and Support links to `LinkSettingsView.swift` using the verified URLs; verify destinations through injected URL-opening tests and confirm both links remain reachable at the largest text size with `just ios-ui-test`, without applying draft settings or opening a browser in tests. All three `SupportLinksUITests` passed with `just ios-ui-test "" CameraGPSLinkUITests/SupportLinksUITests` (`/tmp/camera-gps-link-readiness/support-ui-6.log`, exit 0). Link acceptance checks exact accessible labels, full visible frames, hit targets, destinations, permission state, and retained draft/Cancel behavior, including rejected URL opens. An additional whole-Settings contrast probe failed with blank Form captures/unresolved SwiftUI elements and is not claimed as passing; broad styling experiments were reverted rather than expanding this link task into a Form audit rewrite. Existing home/diagnostics contrast audits remain unchanged. Reopened after the combined `d4bf24a` UI run hit an invalid activation point during scrolling. Navigation now targets actual buttons and polls full viewport geometry before checking hittability, rather than querying hit points on transient off-screen SwiftUI nodes. The renewed combined gate is pending.
...
- [ ] `just check` passes on the final implementation commit; renewed verification is required after adding support links. Previous evidence: source-size checks pass and local toolchain/test results are recorded here. Repeated on committed `42ac9ef` on 2026-09-12: smoke, typecheck, project/source lint, four builds, 152 unit tests, and 34 UI tests, using Xcode 26.6 (`17F113`), Swift 6.3.3, and iOS Simulator 26.5 (`23F77`). Log: `/tmp/camera-gps-link-readiness/committed-gate.log` (exit 0; commit recorded in `committed-gate.sha` alongside it). Full diff review, `git diff --check`, strict Swift formatting, and `actionlint` 1.7.12 passed. GitHub reports the commit signature as verified; local verification against the configured public key also passed.
- [ ] Every required workflow job passes on the final implementation commit, with a hosted run URL and retained verification evidence; no signing secrets or physical-camera access are needed by CI. Previous implementation evidence is in section 3; the support-link changes require a new passing run.
- [ ] Home-screen limitations, cached-location wording, support links, and safe-summary copy behavior pass their unit/UI checks, and the related user/privacy documents match the implementation. Home/summary behavior and matching documents pass; the three focused support-link UI tests pass and obsolete private-hosting notices have been removed. The final combined gate is pending.
