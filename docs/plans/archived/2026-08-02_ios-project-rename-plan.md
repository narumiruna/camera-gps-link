...
- [x] Update root documentation and `justfile` paths, target/scheme names, test selectors, temporary binary/device names, and clean rules; verify old `ios/SonyGeoTag` references are absent from current runtime/tooling documentation and `just --list` succeeds. Verified: root README and every iOS recipe points to `CameraGPSLink`; current runtime/tooling files contain no old path and `just --list` succeeds.
